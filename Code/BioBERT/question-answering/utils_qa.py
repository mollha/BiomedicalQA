...
from transformers.data.processors.squad import SquadExample, SquadV1Processor, SquadV2Processor
    ...
    if os.path.exists(cached_features_file) and not args["overwrite_cache"]:
        ...
        if not args["data_dir"] and ((evaluate and not args["predict_file"]) or (not evaluate and not args["train_file"])):
            ...
            if args["version_2_with_negative"]:
                print("WARNING: tensorflow_datasets does not handle version 2 of SQuAD.")

            # Load the squad dataset if no other dataset provided
